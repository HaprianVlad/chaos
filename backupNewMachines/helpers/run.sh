...
additional_args="--use_vertex_striping --request_batching --batch_size=$bsize --partitions $(($partitions * $partitions * $machines)) --super_partitions $(($partitions)) --fanout $(($partitions * $machines)) --polling_client --polling_server --gather_io_drain --use_async_server --policy_heuristic_new --log_phases --measure_scatter_gather"

...
for i in `seq 137 144`; do scp ~/runner/chaos.zip dco-node$i:/media/${device}/hpgp/branches/.; done
...
for i in `seq 137 144`; do scp ~/runner/helpers.zip dco-node$i:/media/${device}/.; done
